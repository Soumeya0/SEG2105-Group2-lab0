...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    // Declare a reference to the Evaluator class.
    // This object will be responsible for handling all mathematical expression
    // evaluations (e.g., "10 + 5", "20 / 4") outside of the MainActivity.
    // Keeping evaluation logic in a separate class promotes cleaner code,
    // separation of concerns, easier debugging, and allows unit testing of
    // calculation logic independently from the UI.
    Evaluator evaluator;
    ...
    public void onClick(View view) {
        ...
        try {
            // Use the Evaluator class to compute the mathematical expression.
            // The Evaluator.evaluate() method may throw an exception if the expression
            // is invalid (e.g., divide by zero, malformed input). If successful, the
            // result returned by the evaluator is displayed in the TextView.
            String result = evaluator.evaluate(expression);
            textView.setText("Answer = " + result);
        } catch (Exception e) {
            // If something goes wrong during evaluation—such as invalid input,
            // ScriptEngine errors, or unexpected computation issues—this block runs.
            // Instead of crashing the app, we show a safe "Error" message to the user.
